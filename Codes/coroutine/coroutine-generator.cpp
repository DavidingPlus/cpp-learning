...
public:
    ...
    value_type operator()() {  // getter
        _fill();
        full = false;  // 翻转开关，以使promise对象能缓存新数据
...
generator<fibo_valtype> fibonacci(unsigned n) {  // n不得大于94，否则将会导致值溢出
    if (n == 0)
        co_return;  // 如果n==0，本协程将不生成任何结果，直接结束

    co_yield 1;  // 生成第一个值。此后本协程暂停在此处，等待恢复执行
    if (n == 1)
        co_return;  // 如果n==1，本协程将只生成一个结果，然后结束

    co_yield 1;  // 生成第二个值。此后本协程暂停在此处，等待恢复执行
    if (n == 2)
        co_return;  // 如果n==2，现在已生成了两个结果，所以本协程该结束了
...
int main() {
    ...
    for (int i = 0; gen; ++i)
        std::cout << "fib[" << i + 1 << "]=" << gen() << std::endl;

    std::cout << "\nPlease input a range:";
    int a, b;
    std::cin >> a >> b;
    std::cout << "The odd numbers between " << a << " and " << b << " are:" << std::endl;
    auto gen2 = odd(a, b);  // 生成[a,b]之间的所有奇数
    for (int i = 0; gen2; ++i)
        std::cout << i + 1 << ": " << gen2() << std::endl;
