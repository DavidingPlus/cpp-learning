...
protected:
    /**
     * @brief 用于链表的随机访问迭代器基类（模板）。采用了CRTP模式，即它的模板类型参数是它的派生类（模板）
     *
     */
    ...
    protected:
        ...
        virtual void _prev(dataptr_type &p) = 0;

        // 判断指针是否移到链表外
        virtual constexpr bool _is_exceed(dataptr_type p) const = 0;
        virtual constexpr bool _is_next_exceed(dataptr_type p) const = 0;
        virtual constexpr bool _is_prev_exceed(dataptr_type p) const = 0;

        // 将this指针强制转换为iterator*
        inline constexpr iterator_type *cast_this() {
    ...
    public:
         ...
         * @return constexpr iterator_type& 迭代器本身
         */
        constexpr iterator_type &advance(difference_type n = 1) try {
            if (n >= 0) {
        ...
        // 生成一个指向当前项后项的迭代器
        constexpr iterator_type next() const {
            auto itr{*cast_this()};
            _next(itr.p);
            return itr;
        }
        ...
        // 生成一个指向当前项前项的迭代器
        constexpr iterator_type prev() const {
            auto itr{*cast_this()};
            _prev(itr.p);
            return itr;
        }
        ...
        // 重载的!=运算符
        constexpr bool operator!=(iterator_type const &itr) const {
            return p != itr.p;
        }
        ...
        constexpr iterator_type operator+(difference_type n) const {
            return iterator_type(p).advance(n);
        }
...
public:
    ...
    protected:
        ...
        // 内部函数，用于指针的前移或后移
        void _next(dataptr_type &p) override {
            p = p->next;
        }

        void _prev(dataptr_type &p) override {
            p = p->prior;
        }

        // 判断指针是否移到链表外
        constexpr bool _is_exceed(dataptr_type p) const override {
            return p == nullptr;
        }

        constexpr bool _is_next_exceed(dataptr_type p) const override {
            return p->next == nullptr;
        }

        constexpr bool _is_prev_exceed(dataptr_type p) const override {
            return p->prior == nullptr;
        }
    ...
    public:
        ...
        using parent::operator++;  // 一次性引入所有的同名重载版本
        using parent::operator--;
        using parent::operator*;
        using parent::operator+;
        using parent::operator-;
    ...
    class reverse_iterator : private base_list_iterator<reverse_iterator> {
        using parent = base_list_iterator<reverse_iterator>;
        friend parent;
    ...
    /**
     * @brief 定义常量正向迭代器。迭代器是容器的内部类（模板）。这个模板还有瑕疵。
     *
     */
    friend class const_iterator;
    class const_iterator : private base_list_iterator<const_iterator> {
        using parent = base_list_iterator<const_iterator>;
        friend parent;
    ...
    public:
        using parent::base_list_iterator;
        ...
        using parent::operator++;
        using parent::operator--;
        // using parent::operator*; // 不引入原始的版本，因为返回类型不同
        using parent::operator+;
        using parent::operator-;

        /**
         * @brief 常量迭代器返回元素的常量引用。
         *
         * @return constref 元素的常量引用
         */
        constexpr constref operator*() const {
            return p->data;
        }
    };
